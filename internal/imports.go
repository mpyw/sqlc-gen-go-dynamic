...
func (i *importer) queryImports(filename string) fileImports {
	...
	// A query whose template carries directives renders through the runtime, so the file that
	// holds it imports the runtime.
	for _, q := range gq {
		if q.Dynamic {
			pkg[ImportSpec{Path: RuntimeImport}] = struct{}{}
			break
		}
	}
