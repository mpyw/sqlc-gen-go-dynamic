...
type Query struct {
	...
	// Set when the query's template carries /*%if*/ or /*%for*/, in which case SQL is that
	// template rather than finished SQL, and the method renders it per call. A query without
