...
import (
	...
	"github.com/mpyw/sqlc-gen-go-dynamic/internal/inflection"
...
func buildQueries(req *plugin.GenerateRequest, options *opts.Options, structs []Struct) ([]Query, error) {
	...
	for _, query := range req.Queries {
		...
		// A template with directives renders per call, so its SQL is the template and its
		// parameters are a shape rather than a list. Everything else about the query — the row
		// type above, the naming, the comments — is left as it was.
		dyn, err := dynamicQuery(req, options, query)
		if err != nil {
			return nil, err
		}
		if dyn != nil {
